...
| `core/unicode/utf16` (as WTF-8/WTF-16) | REQUIRED for os (all-platforms ruling, July 19, 2026) | The maintainer ruled all-platforms-from-the-start, so the Windows path layer is built WITH os v1, not deferred. Design note from the Zig/Go/Rust convergence: OS strings must round-trip through WTF-8 (a superset of UTF-8 that preserves unpaired surrogates), NOT strict UTF-8 — Windows paths are potentially-ill-formed UTF-16 and strict transcoding would corrupt them. Zig (`lib/std/unicode.zig`), Rust (`library/core/src/wtf8.rs`), and Go (WTF-8 in its Windows syscall layer) all converged on this. So the port is WTF-8↔WTF-16 transcode + the existing utf8 layer stays strict for text; OS-string handling uses the WTF-8 variant. Mechanical surrogate logic, no tables. |
