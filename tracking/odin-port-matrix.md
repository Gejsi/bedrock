...
| `core/unicode/utf16` (as WTF-8/WTF-16) | REQUIRED for os (all-platforms ruling, July 19, 2026); brief complete | The Windows path layer ships WITH os v1. The July 19 brief settled the design: TYPELESS WTF-8 (bytes in views, Zig/Go model — no OsStr type), implemented as a MODE of the landed utf8 codec (Zig's insight: WTF-8 is UTF-8 with the surrogate-half check disabled), plus a well-formed-join concat helper for the surrogate-boundary corner Rust's OsStr type polices. NAMED DEVIATION: Odin's `unicode/utf16` is LOSSY-strict (unpaired surrogates become U+FFFD — would corrupt Windows path round-trips; Odin's own os sidesteps its utf16 via win32 calls), so Bedrock ports Zig's lossless WTF-8 for OS-string duty rather than Odin's utf16. Even Win32's MultiByteToWideChar is lossy by default — WTF-8 is the genuine answer, not a Zig quirk. Separately ruled: NT `\\?\` long-path prefixing (normalize + absolutize + prefix past MAX_PATH) is IN SCOPE for the os Windows backend from the start (Zig's always-prefix model) — punting on >260-char paths would recreate second-class Windows. Design in flight (#74). |
