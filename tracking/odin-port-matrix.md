...
| `core/os` | file and standard-stream slices landed (August 1, 2026) | Complete POSIX/Windows file-handle slice plus borrowed stdin/stdout/stderr byte streams: caller-owned file lifecycle, read/write, native positioned IO, seek, size, atomic create-new, append, portable status plus exact native errors, opaque-byte POSIX paths, WTF-8 Windows paths with internal long-path handling, and redirection-aware standard handles. Remaining families are explicit separate work: metadata and path mutation, environment, arguments, then directory iteration. Process spawning and terminal text conversion remain separate future modules. |
