...
| `core/mem` | partial v1 | Allocators, fixed arenas, rollback stack allocator, tracking allocator, and a now-split `virtual/*` implementation for cross-platform virtual memory, virtual growing/static arenas, temp helpers, trailing guard-page overflow protection, and path-based file mapping landed; specialized allocators still remain. |
