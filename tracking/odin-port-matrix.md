...
| `core/fmt` | PARTIAL REOPEN (July 19, 2026) | Odin's `fmt` (the `any`/RTTI reflection formatter) stays excluded. But the maintainer reopened a BOUNDED sliver: a hand-rolled C-printf-familiar formatter (`%d %u %x %f %s %c %%`) over the landed locale-free `br_format_*` + strings builder — NOT a reflection engine, a fixed-specifier parser. Headline feature: a `br_string_view`-aware `%s` (length+pointer, no cstr conversion — kills the daily interop pain) and locale-freedom by construction. `BR_PRINTF_LIKE(fmt,args)` attribute so GCC/Clang type-check call sites (empty on MSVC). This becomes the library-wide fmt-substitute; a leveled/timestamped logger, if ever wanted, is a thin wrapper over it (the structured-logger design in spec/modules/log.md is SHELVED — the printf is the chosen answer, log.md kept as the fallback design if structured logging is later wanted). Design COMMITTED at spec/modules/print.md (July 19, 2026): br_print_to_buffer/_to_builder + va_list twins, the ratified %.*s + BR_SV_ARG view idiom, %f = C precision-6 semantics, %b deferred (toolchain-dependent attribute recognition), %n refused, stderr convenience cut. Port queued (task #79). |
...
| `core/unicode/utf16` | subtree | Superseded: landed as the lossless WTF-8 codec for OS-string duty (see the `core/unicode/utf16` (as WTF-8) row above). Strict display-transcode (U+FFFD-lossy, Odin's actual utf16 use case) remains unbuilt, on demand. |
