...
It does not own the source memory, it tracks a current read index, and it
supports random-access reads without changing the main cursor.
*/
...
It is valid whenever the current index is greater than zero, not only
immediately after a successful `read_byte`.
*/
